...
let
  ...
  testUniqueVlanIds = 
    ...
    in
    assert lib.length vlanIds == lib.length uniqueIds;
  ...
  testClusterConfig =
    ...
    in
    assert hasApiAddress && hasServerAddr && apiAddressValid;
  ...
  testHostsConfig =
    let
      ...
      checkHost = hostName:
        ...
        in
        assert hasRouter && hasAddresses && hasCluster && hasInfraAddress;
        true;
  ...
  testMultiArchSupport =
    let
      ...
      supportsMultipleArch = lib.all (hostName: 
        ...
        in host ? addresses && host ? router
      ) hostNames;
    in
    assert supportsMultipleArch;
...
in
{
  ...
  inherit testUniqueVlanIds testValidSubnets testClusterConfig 
          testHostsConfig testRouterConfig testClusterApiConsistency
          testMultiArchSupport;
}